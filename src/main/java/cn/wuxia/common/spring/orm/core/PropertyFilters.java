...
public class PropertyFilters {
     ...
     * </p>
     * <code>
     * 	PropertyFilters.build("EQS_propertyName","songlin.li")
     * </code>
     * 
    ...
    public static PropertyFilter build(String expression, String matchValue) {
        ...
        String classType = StringUtils.substring(restrictionsNameAndClassType, restrictionsNameAndClassType.length() - 1,
                restrictionsNameAndClassType.length());
     ...
     * 当参数存在{filter_EQS_property1
     * :value,filter_EQS_property2:''}该形式的时候，将不会创建filter_EQS_property2等于""值的实例
    ...
    /**
     * 从HttpRequest参数中创建PropertyFilter列表,当参数存在{filter_EQS_property1:value,
     * filter_EQS_property2:''} 该形式的时候，将不会创建filter_EQS_property2等于""值的实例 参考
     * {@link PropertyFilters#build(HttpServletRequest, String, boolean)}
     * 
     * @param request HttpServletRequest
